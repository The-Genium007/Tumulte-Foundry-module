...
// ─── Extended HTMLElement for countdown interval ──────────────────────────────

interface CountdownHTMLElement extends HTMLElement {
  _tumulteCountdownInterval?: ReturnType<typeof setInterval> | null
}

// ─── Extended PIXI filter with Tumulte marker ────────────────────────────────

interface TumulteGlowFilterInstance {
...
class TumulteIntegration {
  ...
  socketClient: TumulteSocketClient | null

  // Collectors
  diceCollector: DiceCollector | null
  characterCollector: CharacterCollector | null
  combatCollector: CombatCollector | null
  spellEffectCollector: SpellEffectCollector | null
  ...
  async initialize(): Promise<void> {
    ...
    if (!this.buildUrl) {
      const savedUrl = game.settings.get(MODULE_ID, 'serverUrl') as string
      if (savedUrl) {
  ...
  async runMigrations(): Promise<void> {
    const savedVersion = (game.settings.get(MODULE_ID, 'moduleVersion') as string) || '0.0.0'
    ...
    if (this.buildUrl) {
      const currentUrl = game.settings.get(MODULE_ID, 'serverUrl') as string
  ...
  registerSettings(): void {
    ...
    game.settings.register(MODULE_ID, 'serverUrl', {
      ...
      onChange: (value: unknown) => {
        this.serverUrl = value as string
    ...
    game.settings.register(MODULE_ID, 'sendAllRolls', {
      ...
      onChange: (value: unknown) => {
        if (this.diceCollector) {
          this.diceCollector.setSendAllRolls(value as boolean)
        }
      }
    })
  ...
  setupSocketHandlers(): void {
    ...
    // Connection events
    socketClient.addEventListener('connected', () => {
      this.onConnected()
    })

    socketClient.addEventListener('disconnected', ((event: Event) => {
      this.onDisconnected((event as CustomEvent).detail as Record<string, unknown>)
    }) as EventListener)

    socketClient.addEventListener('reconnecting', ((event: Event) => {
      const { attempt } = (event as CustomEvent).detail as { attempt: number }
      Logger.info(`Reconnecting (attempt ${attempt})...`)
    }) as EventListener)

    socketClient.addEventListener('reconnect-failed', () => {
      ui.notifications.error('Failed to reconnect to Tumulte')
    })

    socketClient.addEventListener('auth-failed', () => {
      ui.notifications.error('Authentication failed. Please re-pair with Tumulte.')
    })

    socketClient.addEventListener('revoked', ((event: Event) => {
      // Connection was revoked (manually by user or due to token invalidation)
      const reason = ((event as CustomEvent).detail as { reason?: string })?.reason || 'Unknown reason'
      new Dialog({
    ...
    socketClient.addEventListener('campaign-deleted', ((event: Event) => {
      // Campaign was deleted on Tumulte, tokens have been cleared
      const campaignName = ((event as CustomEvent).detail as { campaignName?: string })?.campaignName || 'your campaign'
      new Dialog({
    ...
    socketClient.addEventListener('dice-roll-ack', ((event: Event) => {
      const { success, error } = (event as CustomEvent).detail as { success: boolean; error?: string }
      if (!success) {
   ...
   */
  registerSpellEffectVisualHooks(): void {
    // Unified handler for actor sheet hooks -- works with any hook variant
    const actorHookHandler = (hookName: string, app: unknown, html: unknown): void => {
      const appObj = app as { actor?: FoundryActor; document?: FoundryActor; object?: FoundryActor }
      const actor = appObj.actor ?? appObj.document ?? appObj.object
      if (!actor?.items) return // Not an actor sheet -- skip silently

      Logger.info(`Visual hook [${hookName}] fired`, { actor: actor.name, htmlType: (html as object)?.constructor?.name })
    ...
    const itemHookHandler = (_hookName: string, app: unknown, html: unknown): void => {
      const appObj = app as { item?: FoundryItem; document?: FoundryItem; object?: FoundryItem }
      const item = appObj.item ?? appObj.document ?? appObj.object
    ...
    Hooks.on('renderActorSheet', (app: unknown, html: unknown) => actorHookHandler('renderActorSheet', app, html))
    Hooks.on('renderActorSheetV2', (app: unknown, html: unknown) => actorHookHandler('renderActorSheetV2', app, html))
    Hooks.on('renderApplication', (app: unknown, html: unknown) => actorHookHandler('renderApplication', app, html))
    Hooks.on('renderApplicationV2', (app: unknown, html: unknown) => actorHookHandler('renderApplicationV2', app, html))

    // Item sheet hooks (V1 + V2)
    Hooks.on('renderItemSheet', (app: unknown, html: unknown) => itemHookHandler('renderItemSheet', app, html))
    Hooks.on('renderItemSheetV2', (app: unknown, html: unknown) => itemHookHandler('renderItemSheetV2', app, html))
    ...
    const tokenHaloHandler = (token: unknown): void => this._renderMonsterHalo(token as CanvasToken)
    ...
    Hooks.on('updateToken', (tokenDoc: unknown) => {
      const token = (tokenDoc as { object?: CanvasToken })?.object
   ...
   */
  _renderMonsterHalo(token: CanvasToken): void {
    // Check token flag first, then fallback to actor flag (for off-scene NPCs that get placed later)
    const haloFlag = (token.document?.getFlag(MODULE_ID, 'monsterHalo') as MonsterHaloFlag | undefined)
      || (token.actor?.getFlag(MODULE_ID, 'monsterHalo') as MonsterHaloFlag | undefined)
    ...
    const displayObject = (token.mesh || token) as PIXIDisplayObject

    // Remove existing Tumulte glow filter if no flag
    if (!haloFlag?.enabled) {
      if (displayObject?.filters) {
        displayObject.filters = displayObject.filters.filter(
          (f): f is PIXIFilterInstance => f != null && !(f as TumulteGlowFilterInstance)._tumulteMonsterHalo
        )
      }
      return
    }
    ...
    const existing = displayObject?.filters?.find(
      (f): f is PIXIFilterInstance & TumulteGlowFilterInstance =>
        f != null && !!(f as TumulteGlowFilterInstance)._tumulteMonsterHalo
    )
    if (existing) {
    ...
    if (displayObject) {
      try {
        const glow = new TumulteGlowFilter({
          ...
          color: colorInt,
          quality: 0.3,
        }) as unknown as PIXIFilterInstance & TumulteGlowFilterInstance
        glow._tumulteMonsterHalo = true

        if (!displayObject.filters) displayObject.filters = []
        displayObject.filters = [...displayObject.filters.filter((f): f is PIXIFilterInstance => f != null), glow]
   ...
   */
  _resolveHtmlElement(html: unknown): HTMLElement | null {
    ...
    const htmlIndexable = html as { [index: number]: unknown; element?: unknown } | null | undefined
    if (htmlIndexable?.[0] instanceof HTMLElement) return htmlIndexable[0]

    // ApplicationV2 may also pass the element directly as html.element
    if (htmlIndexable?.element instanceof HTMLElement) return htmlIndexable.element
   ...
   */
  _renderItemSheetBanner(item: FoundryItem, html: HTMLElement): void {
    if (!item) return

    const disabledFlag = item.getFlag(MODULE_ID, 'disabled') as DisabledFlag | undefined
   ...
   */
  _highlightSpellEffectsOnSheet(actor: FoundryActor, htmlEl: HTMLElement): void {
    if (!actor) return
    ...
    for (const item of actor.items) {
      ...
      if (disabledFlag) {
        this._applyDisabledVisuals(itemEl as HTMLElement, disabledFlag)
      } else if (effectFlag) {
        this._applyEffectVisuals(itemEl as HTMLElement, effectFlag)
      }
    }
   ...
   */
  _highlightMonsterEffectOnSheet(actor: FoundryActor, htmlEl: HTMLElement): void {
    if (!actor) return
   ...
   */
  _createStatPill(text: string, type: string): HTMLSpanElement {
   ...
   */
  _highlightModifiedStats(htmlEl: HTMLElement, effectFlag: MonsterEffectFlag): void {
    const isBuff = effectFlag.type === 'buff'
  ...
  _findStatElement(
    ...
    const walker = document.createTreeWalker(root, NodeFilter.SHOW_ELEMENT, {
      acceptNode: (node: Node): number => {
        ...
        if (el.tagName === 'INPUT' || el.tagName === 'SELECT') {
          return ((el as HTMLInputElement).value ?? '').trim() === valueStr
    ...
    while ((node = walker.nextNode())) {
      ...
      if (el.closest('.tumulte-monster-banner')) continue
      if (el.classList?.contains('tumulte-stat-modified')) continue
      candidates.push(el)
    }
   ...
   */
  _findStatElementByAttribute(root: HTMLElement, selectors: string[]): HTMLElement | null {
   ...
   */
  _applyStatHighlight(el: HTMLElement, originalValue: number, isBuff: boolean): void {
   ...
   */
  _applyDisabledVisuals(itemEl: HTMLElement, flag: DisabledFlag): void {
   ...
   */
  _applyEffectVisuals(itemEl: HTMLElement, flag: SpellEffectFlag): void {
   ...
   */
  _injectBadge(itemEl: HTMLElement, flag: DisabledFlag | SpellEffectFlag, type: string): void {
    ...
    if (type === 'disabled') {
      ...
      icon.className = 'fas fa-arrow-up'
      const spellFlag = flag as SpellEffectFlag
      if (spellFlag.buffType === 'advantage') {
        label = 'Avantage'
      } else if (spellFlag.bonusValue) {
        label = `+${spellFlag.bonusValue}`
      } else {
        label = 'Amplifié'
      }
    } else {
      icon.className = 'fas fa-arrow-down'
      const spellFlag = flag as SpellEffectFlag
      if (spellFlag.debuffType === 'disadvantage') {
        label = 'Désavantage'
      } else if (spellFlag.penaltyValue) {
        label = `-${spellFlag.penaltyValue}`
      } else {
        label = 'Maudit'
      }
    }
   ...
   */
  _injectCountdown(itemEl: HTMLElement, flag: DisabledFlag): void {
    ...
    const countdownEl = itemEl as CountdownHTMLElement

    const updateCountdown = (): void => {
      const ms = expiresAt - Date.now()
      if (ms <= 0) {
        countdown.textContent = ''
        if (countdownEl._tumulteCountdownInterval) {
          clearInterval(countdownEl._tumulteCountdownInterval)
          countdownEl._tumulteCountdownInterval = null
        }
        return
      }
    ...
    updateCountdown()
    countdownEl._tumulteCountdownInterval = setInterval(updateCountdown, 1000)
   ...
   */
  _registerTooltipHandler(itemEl: HTMLElement, flag: DisabledFlag | SpellEffectFlag, type: string): void {
    // Remove native title
    ...
    const showTooltip = (): void => {
      ...
      if (type === 'disabled') {
        const disabledFlagTyped = flag as DisabledFlag
        if (disabledFlagTyped.expiresAt) {
          const ms = disabledFlagTyped.expiresAt - Date.now()
          if (ms > 0) {
            const min = Math.floor(ms / 60000)
            const sec = Math.floor((ms % 60000) / 1000)
      ...
      if (type === 'buff') {
        ...
        } else if (spellFlag.buffType === 'bonus' && spellFlag.bonusValue) {
          addDetail(`Bonus : +${spellFlag.bonusValue} au prochain jet`)
        }
      } else if (type === 'debuff') {
        const spellFlag = flag as SpellEffectFlag
        if (spellFlag.debuffType === 'disadvantage') {
          addDetail('Type : Désavantage sur le prochain jet')
        } else if (spellFlag.debuffType === 'penalty' && spellFlag.penaltyValue) {
          addDetail(`Pénalité : -${spellFlag.penaltyValue} au prochain jet`)
        }
      }
   ...
   */
  onDisconnected(detail: Record<string, unknown>): void {
   ...
   */
  async completePairing(connectionData: unknown): Promise<boolean> {
    try {
      const result = await this.pairingManager!.completePairing(connectionData as Parameters<PairingManager['completePairing']>[0])
   ...
   */
  startReauthorizationPolling(): void {
   ...
   */
  stopReauthorizationPolling(): void {
    if (this.reauthorizationPollInterval) {
   ...
   */
  async _checkReauthorizationStatus(): Promise<void> {
    try {
   ...
   */
  showRevocationDialog(message?: string): void {
...
Hooks.once('ready', async () => {
  ...
  window.tumulte = new TumulteIntegration() as unknown as NonNullable<typeof window.tumulte>
  await (window.tumulte as unknown as TumulteIntegration).initialize()
})
