...
export interface ExtractedDieResult {
  value: number
  active: boolean
  rerolled: boolean
  exploded: boolean
...
// ─── Extended roll interface for system-specific properties ───────────────────
...
// ─── Extended term interface for runtime properties ───────────────────────────

interface ExtendedDiceTerm extends FoundryDiceTerm {
  constructor?: { name?: string; DENOMINATION?: string }
  expression?: string
  number?: number
 ...
 */
export function extractUniversalRollData(
 ...
 */
function extractTerms(terms: FoundryDiceTerm[]): TermData[] {
  return terms.map(term => extractSingleTerm(term)).filter((t): t is TermData => t !== null)
}

/**
 * Extract data from a single term
 */
function extractSingleTerm(term: FoundryDiceTerm): TermData | null {
  if (!term) return null
 ...
 */
function extractDieTerm(term: ExtendedDiceTerm, className: string): TermData {
  const results: ExtractedDieResult[] = (term.results || []).map((r) => {
    const extR = r as ExtendedDieResult
    return {
      value: extR.result,
 ...
 */
function extractPoolTerm(term: ExtendedDiceTerm, className: string): TermData {
  // Pool terms contain multiple rolls
  const rolls: TermData[][] = (term.rolls || term.terms || []).map((roll) => {
    const rollAny = roll as FoundryDiceTerm & { terms?: FoundryDiceTerm[] }
    if (rollAny.terms) {
  ...
  return {
    ...
    class: className,
    rolls,
    modifiers: (Array.isArray(term.modifiers) ? term.modifiers : []) as string[],
 ...
 */
function extractParentheticalTerm(term: ExtendedDiceTerm, className: string): TermData {
 ...
 */
function extractFunctionTerm(term: ExtendedDiceTerm, className: string): TermData {
  return {
    type: 'function',
    class: className,
 ...
 */
function isDieTerm(term: ExtendedDiceTerm): boolean {
...
function isOperatorTerm(term: ExtendedDiceTerm): boolean {
  const className = term.constructor?.name || ''
  return className === 'OperatorTerm' || term.operator !== undefined
}

function isNumericTerm(term: ExtendedDiceTerm): boolean {
  const className = term.constructor?.name || ''
  return className === 'NumericTerm' || (term.number !== undefined && term.faces === undefined)
}

function isPoolTerm(term: ExtendedDiceTerm): boolean {
  const className = term.constructor?.name || ''
  return className === 'PoolTerm' || className.includes('Pool')
}

function isParentheticalTerm(term: ExtendedDiceTerm): boolean {
  const className = term.constructor?.name || ''
  return className === 'ParentheticalTerm' || className.includes('Parenthetical')
}

function isFunctionTerm(term: ExtendedDiceTerm): boolean {
  const className = term.constructor?.name || ''
 ...
 */
function extractFlatDiceResults(terms: TermData[]): number[] {
 ...
 */
function isPoolBasedSystem(systemId: string): boolean {
  const poolSystems: string[] = [
 ...
 */
function isNarrativeDiceSystem(systemId: string): boolean {
  const narrativeSystems: string[] = [
...
function extractPoolResults(
  ...
  if (roll.options?.successes !== undefined) {
    successes = roll.options.successes as number
  }
  if (roll.options?.failures !== undefined) {
    failures = roll.options.failures as number
  }
    ...
    for (const term of roll.terms || []) {
      if (term.results) {
        for (const result of term.results) {
          ...
          if (extResult.success) successes++
          if (extResult.failure) failures++
        }
      }
    }
  }
    ...
    for (const term of roll.terms || []) {
      if (term.faces === 10 && term.results) {
        for (const result of term.results) {
          ...
          if (result.result === 10 && (roll.options?.doubleSuccess as boolean | undefined)) successes++
        }
      }
    }
  }
 ...
 */
function extractNarrativeSymbols(roll: ExtendedRoll, _systemId: string): SymbolResult[] {
  const symbols: SymbolResult[] = []

  // Try to extract from roll options or results
  if (roll.options?.symbols) {
    return roll.options.symbols as SymbolResult[]
  }
...
function detectCritical(
  ...
  systemId: string,
  systemData: SystemData
): { isCritical: boolean; criticalType: string | null } {
  ...
  if (systemId === 'dnd5e' || !isPoolBasedSystem(systemId)) {
    ...
    const critThreshold = (roll.options?.critical as number | undefined) || 20
    const fumbleThreshold = (roll.options?.fumble as number | undefined) || 1
