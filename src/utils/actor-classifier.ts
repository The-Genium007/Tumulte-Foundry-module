 ...
 */
export interface SystemActorConfig {
  pcTypes: string[]
  npcTypes: string[]
  monsterTypes: string[]
  excludedTypes: string[]
  /**
   * Optional custom classifier to distinguish NPCs from Monsters
   * when an actor type appears in both lists.
   * null means the type-based mapping is sufficient.
   */
  classifyNpcVsMonster: ((actor: FoundryActor) => CharacterType) | null
}

/** Extended actor with hasPlayerOwner (runtime Foundry property) */
interface ClassifiableActor extends FoundryActor {
  hasPlayerOwner?: boolean
}

// ─── System Configurations ────────────────────────────────────────────────────

const SYSTEM_ACTOR_CONFIG: Record<string, SystemActorConfig> = {
  // ============================================
  // D&D 5th Edition
  // ============================================
  'dnd5e': {
     ...
     */
    classifyNpcVsMonster: (actor: FoundryActor): CharacterType => {
      const cr = (actor.system?.details as Record<string, unknown> | undefined)?.cr as number ?? 0
      ...
      const creatureType = typeObj?.value as string | undefined
      ...
      if (creatureType && monsterCreatureTypes.includes(creatureType.toLowerCase())) {
        return 'monster'
      }
  ...
  'pf2e': {
     ...
     */
    classifyNpcVsMonster: (actor: FoundryActor): CharacterType => {
      const traits = actor.system?.traits as Record<string, unknown> | undefined
      const rarity = traits?.rarity as string | undefined
      const details = actor.system?.details as Record<string, unknown> | undefined
      const levelObj = details?.level as Record<string, unknown> | undefined
      const level = (levelObj?.value as number) ?? 0
  ...
  'swade': {
     ...
     */
    classifyNpcVsMonster: (actor: FoundryActor): CharacterType => {
      const isWildCard = (actor.system?.wildcard as boolean) ?? false
  ...
  'cyberpunk-red-core': {
     ...
     */
    classifyNpcVsMonster: (actor: FoundryActor): CharacterType => {
      const role = actor.system?.role as Record<string, unknown> | string | undefined
      const hasRole = typeof role === 'object' && role !== null
        ? (role as Record<string, unknown>).value
        : role
  ...
  'sfrpg': {
    ...
    classifyNpcVsMonster: (actor: FoundryActor): CharacterType => {
      const details = actor.system?.details as Record<string, unknown> | undefined
      const cr = (details?.cr as number) ?? 0
      return cr >= 1 ? 'monster' : 'npc'
    }
  },
  ...
  'starwarsffg': {
     ...
     */
    classifyNpcVsMonster: (actor: FoundryActor): CharacterType => {
      const characteristics = actor.system?.characteristics as Record<string, unknown> | undefined
      const npcType = characteristics?.npctype as string | undefined
  ...
  'lancer': {
     ...
     */
    classifyNpcVsMonster: (actor: FoundryActor): CharacterType => {
      const tier = (actor.system?.tier as number) ?? 0
 ...
 * Covers the most common naming conventions across RPG systems.
 */
const GENERIC_CONFIG: SystemActorConfig = {
 ...
 */
export function getSystemConfig(systemId: string | null = null): SystemActorConfig {
  const id = systemId || game.system?.id
  if (id) {
    const config = SYSTEM_ACTOR_CONFIG[id]
 ...
 */
export function shouldSyncActor(actor: FoundryActor | null): boolean {
 ...
 * 5. Fallback -> NPC
 */
export function classifyActor(actor: ClassifiableActor | null): CharacterType {
  if (!actor) return 'npc'
 ...
 */
export function hasSystemSupport(): boolean {
  return Object.prototype.hasOwnProperty.call(SYSTEM_ACTOR_CONFIG, game.system?.id)
}
