...
interface PairingStatusResponse {
  status: 'completed' | 'expired' | 'pending' | 'not_found'
...
interface PairingCompleteCallbackData {
  success: boolean
  connectionId: string
  worldId?: string
  worldName?: string
...
type PairingCompleteCallback = (data: PairingCompleteCallbackData) => void
type PairingExpiredCallback = () => void

// ─── Class ────────────────────────────────────────────────────────────────────

export class PairingManager {
  private worldId: string
  private tumulteUrl: string
  private tokenStorage: TokenStorage

  private currentPairing: CurrentPairing | null
  private pairingTimeout: ReturnType<typeof setTimeout> | null
  private pollingInterval: ReturnType<typeof setInterval> | null
  private onPairingComplete: PairingCompleteCallback | null
  private onPairingExpired: PairingExpiredCallback | null
   ...
   */
  async checkPairingStatus(): Promise<void> {
   ...
   */
  async completePairingFromBackend(data: PairingStatusResponse): Promise<PairingCompleteResult> {
    try {
      // Stop polling
      this.stopPolling()
   ...
   */
  setPairingTimeout(duration: number): void {
    if (this.pairingTimeout) {
      clearTimeout(this.pairingTimeout)
    }
   ...
   */
  async completePairing(connectionData: ConnectionData): Promise<PairingCompleteResult> {
    try {
   ...
   */
  onComplete(callback: PairingCompleteCallback): void {
   ...
   */
  onExpired(callback: PairingExpiredCallback): void {
