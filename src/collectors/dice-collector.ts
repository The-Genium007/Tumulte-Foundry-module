...
import type { ExtractedRollData } from '../utils/system-adapters.js'
...
/** Roll data enriched with campaign identifiers before sending */
interface EnrichedRollData extends ExtractedRollData {
  worldId: string
  campaignId: string
}
...
export class DiceCollector {
  ...
  initialize(): void {
    ...
    Hooks.on('createChatMessage', this.onChatMessage.bind(this) as (...args: unknown[]) => void)
  ...
  loadSettings(): void {
    try {
      this.sendAllRolls = game.settings.get(MODULE_ID, 'sendAllRolls') as boolean
    } catch {
      this.sendAllRolls = false
    }
  }
  ...
  registerSystemHooks(): void {
    ...
    switch (systemId) {
      ...
      case 'pf2e':
        ...
        Hooks.on('pf2e.rollCheck', (...args: unknown[]) => {
          const actor = args[0] as { name?: string } | undefined
          const type = args[2] as string | undefined
   ...
   */
  async onChatMessage(message: FoundryChatMessage, _options: unknown, _userId: string): Promise<void> {
   ...
   */
  async processRoll(message: FoundryChatMessage, roll: FoundryRoll): Promise<void> {
    try {
      ...
      const extractedData = this.systemAdapter!.extractRollData(message, roll)

      // Add world/campaign info
      const rollData: EnrichedRollData = {
        ...extractedData,
        worldId: game.world.id,
        campaignId: game.world.id, // Use world ID as campaign ID
      }
   ...
   */
  shouldSendRoll(rollData: EnrichedRollData): boolean {
   ...
   */
  setSendAllRolls(value: boolean): void {
