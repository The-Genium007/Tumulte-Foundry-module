...
/** Spell effect flag data stored on Foundry items via setFlag */
...
/** Pending consumption entry tracked between pre-roll and post-use hooks */
...
/** Payload sent to the backend when an effect is consumed */
interface EffectConsumedPayload {
  spellId: string
  spellName: string
...
/** dnd5e 5.x roll configuration (D20RollProcessConfiguration / DamageRollProcessConfiguration) */
interface Dnd5eRollConfig {
  subject?: Dnd5eActivity
  item?: FoundryItem
  advantage?: boolean
  disadvantage?: boolean
  rolls?: RollConfiguration[]
  rollConfigs?: RollConfiguration[]
}

/** Individual roll configuration within a process config */
interface RollConfiguration {
  parts?: string[]
  options?: Record<string, unknown>
}

/** dnd5e 5.x Activity object — has a reference to its parent Item */
interface Dnd5eActivity {
  item?: FoundryItem
}

/** dnd5e 4.x legacy use config */
interface Dnd5eLegacyConfig {
  rollConfigs?: LegacyRollConfig[]
}

/** dnd5e 4.x legacy roll config entry */
interface LegacyRollConfig {
  parts?: string[]
  options?: LegacyRollOptions
}

/** dnd5e 4.x legacy roll options */
interface LegacyRollOptions {
  advantage?: boolean
  disadvantage?: boolean
  [key: string]: unknown
}

/** PF2e pre-roll check context */
interface Pf2eCheckContext {
  item?: FoundryItem
  options?: string[]
}

/** WFRP4e roll test data */
interface Wfrp4eTestData {
  item?: FoundryItem
  testModifier?: number
}
...
const PRE_ROLL_SYSTEMS: string[] = ['dnd5e', 'pf2e', 'wfrp4e']
...
export class SpellEffectCollector {
  ...
  private _pendingConsumption: Map<string, PendingConsumption>
   ...
   */
  private _onDnd5ePreRollAttack(config: Dnd5eRollConfig, _dialog: unknown, _message: unknown): boolean {
   ...
   */
  private _onDnd5ePreRollDamage(config: Dnd5eRollConfig, _dialog: unknown, _message: unknown): boolean {
    const item = this._resolveItemFromConfig(config)
    if (!item) return true
   ...
   */
  private async _onDnd5ePostUseActivity(activity: Dnd5eActivity | undefined, _usageConfig: unknown, _results: unknown): Promise<void> {
  ...
  private _onDnd5eLegacyPreUseItem(item: FoundryItem, config: Dnd5eLegacyConfig, _options: unknown): boolean {
    ...
    if (effect.type === 'buff') {
      if (effect.buffType === 'advantage') {
        if (config.rollConfigs) {
          config.rollConfigs.forEach((rc: LegacyRollConfig) => {
  ...
  private async _onDnd5eLegacyUseItem(item: FoundryItem, _config: Dnd5eLegacyConfig, _options: unknown): Promise<void> {
   ...
   */
  private _onPf2ePreRollCheck(_roll: unknown, context: Pf2eCheckContext): boolean {
   ...
   */
  registerUniversalChatHooks(): void {
    Hooks.on('createChatMessage', (...args: unknown[]) => {
      const message = args[0] as FoundryChatMessage
      try {
        void this._onUniversalChatMessage(message)
      } catch (error) {
   ...
   */
  private async _onUniversalChatMessage(message: FoundryChatMessage): Promise<void> {
   ...
   */
  private _getSpellItemTypes(): string[] {
   ...
   */
  private _resolveItemFromConfig(config: Dnd5eRollConfig): FoundryItem | null {
   ...
   */
  private _resolveItemFromMessage(message: FoundryChatMessage): FoundryItem | null {
   ...
   */
  private _extractItemIdFromFlags(message: FoundryChatMessage): string | null {
    try {
      // Foundry core flag
      const coreItemId = message.getFlag?.('core', 'itemId') as string | undefined
      ...
      const pf2eUuid = pf2eOrigin?.['uuid'] as string | undefined
      if (pf2eUuid) {
        const parts = pf2eUuid.split('.')
        const itemIdx = parts.indexOf('Item')
        if (itemIdx >= 0 && parts[itemIdx + 1]) return parts[itemIdx + 1]!
      }

      // wfrp4e
      const wfrp4eFlags = flags['wfrp4e'] as Record<string, unknown> | undefined
      if (wfrp4eFlags?.['itemId']) return wfrp4eFlags['itemId'] as string

      // CoC7
      const coc7Flags = flags['CoC7'] as Record<string, unknown> | undefined
      if (coc7Flags?.['itemId']) return coc7Flags['itemId'] as string

      // SWADE
      const swadeFlags = flags['swade'] as Record<string, unknown> | undefined
      if (swadeFlags?.['itemId']) return swadeFlags['itemId'] as string

      // Generic: some systems use flags.core.sourceId or flags.item.id
      const coreFlags = flags['core'] as Record<string, unknown> | undefined
      if (coreFlags?.['sourceId']) {
        const parts = (coreFlags['sourceId'] as string).split('.')
        const itemIdx = parts.indexOf('Item')
        if (itemIdx >= 0 && parts[itemIdx + 1]) return parts[itemIdx + 1]!
      }
    } catch {
   ...
   */
  private _resolveItemByNameMatch(actor: FoundryActor, message: FoundryChatMessage): FoundryItem | null {
    const spellTypes = this._getSpellItemTypes()
   ...
   */
  private _addPartsToRolls(config: Dnd5eRollConfig, partString: string): void {
   ...
   */
  private async _sendConsumptionMessage(item: FoundryItem, effect: SpellEffect): Promise<void> {
   ...
   */
  private async _sendEffectAnnotation(item: FoundryItem, effect: SpellEffect, modifier: string): Promise<void> {
    const label = effect.type === 'buff' ? 'Amplification' : 'Malédiction'
   ...
   */
  private async _sendAdvantageNotification(item: FoundryItem, effect: SpellEffect, type: 'advantage' | 'disadvantage'): Promise<void> {
   ...
   */
  private _notifyBackend(item: FoundryItem, effect: SpellEffect): void {
    const payload: EffectConsumedPayload = {
      spellId: item.id,
      spellName: item.name,
      effectType: effect.type,
      requestId: effect.requestId,
    }
    this.socket.emit('spell:effect:consumed', payload)
  }
}
