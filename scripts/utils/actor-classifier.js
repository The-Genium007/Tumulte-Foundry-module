...
const SYSTEM_ACTOR_CONFIG = {
    ...
    'dnd5e': {
        ...
        classifyNpcVsMonster: (actor) => {
            ...
            const typeObj = details?.type;
            const creatureType = typeObj?.value;
    ...
    'pf2e': {
        ...
        classifyNpcVsMonster: (actor) => {
            ...
            const details = actor.system?.details;
            const levelObj = details?.level;
            const level = levelObj?.value ?? 0;
    ...
    'starwarsffg': {
        ...
        classifyNpcVsMonster: (actor) => {
            ...
            const npcType = characteristics?.npctype;
            if (npcType === 'minion')
                return 'monster';
            return 'npc';
        }
...
export function shouldSyncActor(actor) {
    ...
    const isExcluded = config.excludedTypes.some(excluded => actorType === excluded.toLowerCase());
    if (isExcluded) {
...
export function classifyActor(actor) {
    ...
    const isPcType = config.pcTypes.some(pcType => actorType === pcType.toLowerCase());
    if (isPcType) {
    ...
    const isMonsterType = config.monsterTypes.some(monsterType => actorType === monsterType.toLowerCase());
    if (isMonsterType) {
    ...
    const isNpcType = config.npcTypes.some(npcType => actorType === npcType.toLowerCase());
    if (isNpcType) {
...
//# sourceMappingURL=actor-classifier.js.map