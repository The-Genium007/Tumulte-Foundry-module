...
export class TumulteSocketClient extends EventTarget {
    ...
    scheduleReconnect(delay = null) {
        ...
        // Exponential backoff
        const actualDelay = delay ?? Math.min(this.reconnectDelay * Math.pow(2, this.reconnectAttempts), this.maxReconnectDelay);
    ...
    async checkReauthorizationStatus() {
        try {
            ...
            const response = await fetch(`${this.serverUrl}/webhooks/foundry/reauthorization-status?worldId=${encodeURIComponent(this.worldId)}`, { method: 'GET', headers });
            if (!response.ok) {
                const error = await response.json().catch(() => ({ error: 'Unknown error' }));
    ...
    getTargetableItemTypes() {
        if (!this._itemCategories)
            return null;
        return [...new Set(this._itemCategories
    ...
    async _applySpellDisable(actor, spell, effect, requestId) {
        ...
        const originalPrepared = spell.system?.prepared
        ...
        if (!this._spellDisableTimers)
            this._spellDisableTimers = new Map();
     ...
     */
    async _applySpellBuff(actor, spell, effect, _requestId) {
        const triggeredBy = effect.triggeredBy || 'le chat';
     ...
     */
    async _applySpellDebuff(actor, spell, effect, _requestId) {
        const triggeredBy = effect.triggeredBy || 'le chat';
    ...
    async handleCommandApplyMonsterEffect(data) {
        ...
        try {
            ...
            if (actorClassification === 'pc') {
                ...
                throw new Error(`Cannot apply monster effect to player character "${actor.name}". ` +
        ...
        switch (systemId) {
            ...
            case 'CoC7':
                return {
                    ...
                    read: (a) => ({
                        ...
                        hpTemp: 0,
                        ac: a.system?.attribs?.armor?.value ?? null,
                    }),
                };
            case 'wfrp4e':
                return {
                    ...
                    read: (a) => {
                        ...
                        if (armourObj) {
                            ...
                            for (const zone of zones) {
                                const zoneVal = armourObj[zone];
                                if (typeof zoneVal === 'number') {
                                    total += zoneVal;
                                    count++;
                                }
                            }
                            if (count > 0)
                                avgArmour = Math.round(total / count);
                        }
                        return {
                            hp: wounds?.value ?? 0,
                            hpMax: wounds?.max ?? 10,
            ...
            case 'alienrpg':
                return {
                    ...
                    read: (a) => ({
                        ...
                        hpTemp: 0,
                        ac: a.system?.header?.armor?.value ?? null,
                    }),
                };
            case 'starwarsffg':
                return {
            ...
            case 'wod5e':
                return {
                    ...
                    read: (a) => ({
                        hp: a.system?.health?.value ?? 0,
                        hpMax: a.system?.health?.max ?? 10,
                        hpTemp: 0,
                        ac: a.system?.willpower?.max ?? null,
                    }),
                };
            default:
                return {
     ...
     */
    _getMonsterEffectLabel(effectType, effect) {
        const systemId = game.system?.id;
        if (effectType === 'buff') {
            switch (systemId) {
                case 'k4lt':
     ...
     */
    async _applyNarrativeMonsterEffect(actor, token, effect, effectType, requestId) {
        const systemId = game.system?.id;
    ...
    async _applyMonsterBuff(actor, token, effect, requestId) {
        ...
        const haloData = { enabled: true, color: highlightColor, type: 'buff' };
        await actor.setFlag(MODULE_ID, 'monsterHalo', haloData);
        if (token) {
            await token.document?.setFlag(MODULE_ID, 'monsterHalo', haloData);
            requestAnimationFrame(() => token.refresh());
    ...
    async _applyMonsterDebuff(actor, token, effect, requestId) {
        ...
        const haloData = { enabled: true, color: highlightColor, type: 'debuff' };
        await actor.setFlag(MODULE_ID, 'monsterHalo', haloData);
        if (token) {
            await token.document?.setFlag(MODULE_ID, 'monsterHalo', haloData);
            requestAnimationFrame(() => token.refresh());
