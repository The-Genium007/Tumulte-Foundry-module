...
export class SpellEffectCollector {
    ...
    _registerDnd5eV5Hooks() {
        ...
        try {
            Hooks.on('dnd5e.postUseActivity', (...args) => {
                const activity = args[0];
                const usageConfig = args[1];
    ...
    _registerDnd5eV4Hooks() {
        try {
            Hooks.on('dnd5e.preUseItem', (...args) => {
                const item = args[0];
                const config = args[1];
     ...
     */
    _onDnd5ePreRollAttack(config, _dialog, _message) {
     ...
     */
    _onDnd5ePreRollDamage(config, _dialog, _message) {
        const item = this._resolveItemFromConfig(config);
        if (!item)
            return true;
        const effect = item.getFlag(MODULE_ID, FLAG_KEY);
     ...
     */
    async _onDnd5ePostUseActivity(activity, _usageConfig, _results) {
    ...
    // ----------------------------------------
    _onDnd5eLegacyPreUseItem(item, config, _options) {
        if (item.type !== 'spell')
            return true;
        const effect = item.getFlag(MODULE_ID, FLAG_KEY);
        if (!effect)
    ...
    async _onDnd5eLegacyUseItem(item, _config, _options) {
        if (item.type !== 'spell')
            return;
        const effect = item.getFlag(MODULE_ID, FLAG_KEY);
        if (!effect)
    ...
    registerPf2eHooks() {
        try {
            Hooks.on('pf2e.preRollCheck', (...args) => {
                const roll = args[0];
     ...
     */
    _onPf2ePreRollCheck(_roll, context) {
    ...
    registerWfrp4eHooks() {
        try {
            Hooks.on('wfrp4e.rollTest', (...args) => {
                const testData = args[0];
                const _cardOptions = args[1];
                const item = testData.item;
                if (!item)
                    return;
                const spellTypes = this._getSpellItemTypes();
                if (!spellTypes.includes(item.type))
                    return;
                const effect = item.getFlag(MODULE_ID, FLAG_KEY);
                if (!effect)
                    return;
    ...
    _extractItemIdFromFlags(message) {
        try {
            ...
            const dnd5eUse = dnd5eFlags?.['use'];
            if (dnd5eUse?.['itemId'])
                return dnd5eUse['itemId'];
            const dnd5eItem = dnd5eFlags?.['item'];
            if (dnd5eItem?.['id'])
                return dnd5eItem['id'];
            // v5.x: activity-based resolution
            const dnd5eActivity = dnd5eFlags?.['activity'];
            if (dnd5eActivity?.['itemId'])
                return dnd5eActivity['itemId'];
            // v5.x: UUID resolution from use.itemUuid
            const itemUuid = dnd5eUse?.['itemUuid'];
            if (itemUuid) {
                const uuidParts = itemUuid.split('.');
                const itemIdx = uuidParts.indexOf('Item');
                if (itemIdx >= 0 && uuidParts[itemIdx + 1])
                    return uuidParts[itemIdx + 1];
            }
            // pf2e: origin UUID like "Actor.xxx.Item.yyy"
            const pf2eFlags = flags['pf2e'];
            const pf2eOrigin = pf2eFlags?.['origin'];
            const pf2eUuid = pf2eOrigin?.['uuid'];
            if (pf2eUuid) {
            ...
            // wfrp4e
            const wfrp4eFlags = flags['wfrp4e'];
            if (wfrp4eFlags?.['itemId'])
                return wfrp4eFlags['itemId'];
            // CoC7
            const coc7Flags = flags['CoC7'];
            if (coc7Flags?.['itemId'])
                return coc7Flags['itemId'];
            // SWADE
            const swadeFlags = flags['swade'];
            if (swadeFlags?.['itemId'])
                return swadeFlags['itemId'];
            // Generic: some systems use flags.core.sourceId or flags.item.id
            const coreFlags = flags['core'];
            if (coreFlags?.['sourceId']) {
                const parts = coreFlags['sourceId'].split('.');
                const itemIdx = parts.indexOf('Item');
                if (itemIdx >= 0 && parts[itemIdx + 1])
                    return parts[itemIdx + 1];
            }
...
//# sourceMappingURL=spell-effect-collector.js.map