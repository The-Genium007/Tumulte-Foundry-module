...
class TumulteIntegration {
    ...
    constructor() {
        ...
        const configuredUrl = 'https://api-app.tumulte.app';
    ...
    setupSocketHandlers() {
        ...
        socketClient.addEventListener('disconnected', ((event) => {
            this.onDisconnected(event.detail);
        }));
        socketClient.addEventListener('reconnecting', ((event) => {
        ...
        socketClient.addEventListener('revoked', ((event) => {
        ...
        socketClient.addEventListener('campaign-deleted', ((event) => {
        ...
        socketClient.addEventListener('dice-roll-ack', ((event) => {
    ...
    registerSpellEffectVisualHooks() {
        ...
        const actorHookHandler = (hookName, app, html) => {
            ...
            if (!actor?.items)
                return; // Not an actor sheet -- skip silently
        ...
        const itemHookHandler = (_hookName, app, html) => {
            const appObj = app;
            const item = appObj.item ?? appObj.document ?? appObj.object;
            if (!item?.getFlag)
                return; // Not a valid item sheet
            const el = this._resolveHtmlElement(html);
            if (!el)
                return;
    ...
    _renderMonsterHalo(token) {
        ...
        if (!haloFlag?.enabled) {
            if (displayObject?.filters) {
                displayObject.filters = displayObject.filters.filter((f) => f != null && !f._tumulteMonsterHalo);
            }
            return;
        }
        ...
        const existing = displayObject?.filters?.find((f) => f != null && !!f._tumulteMonsterHalo);
        if (existing) {
        ...
        if (displayObject) {
            try {
                ...
                displayObject.filters = [...displayObject.filters.filter((f) => f != null), glow];
    ...
    _resolveHtmlElement(html) {
        ...
        const htmlIndexable = html;
        if (htmlIndexable?.[0] instanceof HTMLElement)
            return htmlIndexable[0];
        // ApplicationV2 may also pass the element directly as html.element
        if (htmlIndexable?.element instanceof HTMLElement)
            return htmlIndexable.element;
    ...
    _highlightSpellEffectsOnSheet(actor, htmlEl) {
        ...
        for (const item of actor.items) {
            ...
            const itemEl = htmlEl.querySelector(`[data-item-id="${item.id}"]`) ||
    ...
    _highlightMonsterEffectOnSheet(actor, htmlEl) {
        ...
        if (htmlEl.querySelector('.tumulte-monster-banner'))
            return;
        // -- Build the banner --
     ...
     */
    _findStatElement(root, currentValue, _originalValue, attrSelectors, labelKeywords) {
        ...
        while ((node = walker.nextNode())) {
            ...
            if (el.closest('.tumulte-monster-banner'))
                continue;
            if (el.classList?.contains('tumulte-stat-modified'))
                continue;
            candidates.push(el);
        }
    ...
    _applyDisabledVisuals(itemEl, flag) {
        ...
        if (flag.expiresAt)
            this._injectCountdown(itemEl, flag);
    ...
    _injectBadge(itemEl, flag, type) {
        ...
        if (itemEl.querySelector('.tumulte-badge'))
            return;
...
//# sourceMappingURL=tumulte.js.map